...
if [ ! -n "$9" ] ;then
  nohup ffmpeg -y -loglevel debug  -f x11grab  -video_size $5 -framerate $7  -i :76 -f alsa -ac 2 -ar 44100 -i default -b:v $8 /var/output/test.mp4  > /tmp/ffmpeg.log 2>&1 &
else
  nohup ffmpeg -y -loglevel debug \
  -f x11grab -video_size $5 -framerate $7 -i :76 \
  -f alsa -ac 2 -ar 44100 -i default -b:v $8 \
  -f mp4 -map 0 -map 1 /var/output/test.mp4 \
  -f flv -map 0 -map 1 rtmp://$9 > /tmp/ffmpeg.log 2>&1 &
fi
