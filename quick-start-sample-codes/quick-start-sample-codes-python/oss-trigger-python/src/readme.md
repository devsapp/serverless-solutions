...
- 代码测试
  - 运行 `s cli fc-event oss` 生成 OSS Trigger 的 Event 样例参数，生成的文件位于 `event-template/test-oss.json`
  ...
  ```
  - 使用测试样例进行触发测试，运行 `s cli fc invoke --service-name ${serviceNamme} --function-name ${functionName} --event-file event-template/test-oss.json --region ${regionName}`
